...
const IncomeByCategoryChart: React.FC<IncomeByCategoryProps> = ({ transactions }) => {
  ...
  const total = useMemo(
    () => incomeCategories.reduce((sum, cat) => sum + cat.value, 0),
    [incomeCategories]
  );
  ...
  return (
    <Card className="bg-slate-800 bg-opacity-50 backdrop-blur-sm border border-slate-700 rounded-2xl">
      <CardHeader>
        <CardTitle className="text-white text-xl font-bold">Income by Category</CardTitle>
      </CardHeader>
      <CardContent>
